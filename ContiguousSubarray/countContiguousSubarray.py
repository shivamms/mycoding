# Facebook
# array manipulation
# check back and forth of array
# use dictionary to use past history
...
#-----Simple approach
def getCount(arr, n, i):
  count = 0
  for j in range(i, -1, -1): # i=0 i=1
    if arr[i] < arr[j]: # 3 < 3 # 4 < 4, 4 < 3 
      break
    count += 1 # 1 # 2
  for j in range(i, n): # 3 < 4 # 4 < 1, 4 < 6
    if arr[i] < arr[j]:
      break
    count += 1 # 1 # 3
  return count - 1# 1 # 3
...
def count_subarrays(arr):
  ...
  result = []
  for i in range(n): 
    result.append(getCount(arr, n, i)) # arr, 5, 0 => arr, 5, 1
  return result
